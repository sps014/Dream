...
*   **Don't Repeat Yourself (DRY):**
    ...
    *   The standard library under `src/stdlib/system/*.dream` is the single source of truth. Packages are registered in `STD_PACKAGES` / `BOOTSTRAP_PACKAGES` (`src/stdlib/mod.rs`). Both the main compiler and `dream-lsp` reuse these exact files. User code imports opt-in packages (`import system.net;`, etc.).
