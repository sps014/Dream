...
//! method-only placement — is already validated by [`crate::attributes`]), checks role-specific
...
impl<'a> Analyzer<'a> {
    ...
    pub(in crate::semantics::analyzer) fn validate_and_register_protocol_hook(
        &mut self,
        target_type_str: &str,
    ...
    pub(in crate::semantics::analyzer) fn protocol_hook(
        &mut self,
        obj_type: &Type,
        role: ProtocolRole,
        diagnostics: &mut DiagnosticBag,
        ...
        let mono_name = crate::syntax::nodes::types::mangle_generic(&base_name, &generic_args);
        self.protocol_hooks
            .get(&mono_name)
